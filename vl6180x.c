 ...
 *  失败 1 
 */
u8 VL6180X_WriteBytes(u8 I2C_addr,u16 index,u8 dat)
{
  ...
  u8 Index_L = (u8)(index & 0xff);
	...
	IIC_Send_Byte(Index_H); //TDOS'register
  ...
  IIC_Send_Byte(Index_L); //TDOS'register
	if (IIC_Wait_Ack()==0) 
	{
		goto err;
	}
        
	IIC_Send_Byte(dat); //发送数据字节
 ...
 * 通过I2C总线读出某一寄存器的数据
 */
u8 VL6180x_ReadBytes(u8 I2C_addr, u16 index) 
{
	u8 dat;
  u8 Index_H = (u8)(index >> 8);
  u8 Index_L = (u8)(index & 0xff);
 ...
 * 通过I2C总线读出某一寄存器的数据
 */
u16 VL6180x_Read_Two_Bytes(u8 I2C_addr, u16 index) 
{
	u8 t;
  u16 data_res;
	u8 Index_H = (u8)(index >> 8);
